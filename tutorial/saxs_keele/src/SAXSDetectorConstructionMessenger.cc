  ...
  : G4UImessenger(), fDetector(detconstr)
{
  ...
  fPhantomBoxCmd = new G4UIcmdWithABool("/det/setPhantomBox", this);
  fPhantomBoxCmd->SetGuidance("Use a rectangular slab; diameter sets beam-axis thickness and height sets both lateral sizes");
  fPhantomBoxCmd->SetParameterName("PhantomBox", true);
  fPhantomBoxCmd->SetDefaultValue(false);
  fPhantomBoxCmd->AvailableForStates(G4State_PreInit, G4State_Idle);
...
void SAXSDetectorConstructionMessenger::SetNewValue(G4UIcommand* command, G4String newValue)
{
  ...
  if (command == fPhantomBoxCmd) {
    fDetector->SetPhantomBox(fPhantomBoxCmd->GetNewBoolValue(newValue));
  }
