...
void SAXSDetectorConstruction::DefineMaterials()
{
  ...
  if (std::abs(comp[0] + comp[1] + comp[2] + comp[3] - 1.) > 1.e-12) {
...
G4VPhysicalVolume* SAXSDetectorConstruction::Construct()
{
  ...
  // For a box, diameter is the beam-axis thickness and height is both lateral sizes.
  G4VSolid* PhantomSolid = nullptr;
  G4RotationMatrix* PhantomRotationMatrix = nullptr;
  if (fPhantomBox) {
    PhantomSolid = new G4Box("PhantomSolid", fPhantomHeight * 0.5, fPhantomHeight * 0.5,
                             fPhantomDiameter * 0.5);
  }
  else {
    PhantomSolid = new G4Tubs("PhantomSolid", 0., fPhantomDiameter * 0.5,
                              fPhantomHeight * 0.5, 0. * deg, 360. * deg);
    PhantomRotationMatrix = new G4RotationMatrix(0., 0., 0.);
    PhantomRotationMatrix->rotateX(90. * CLHEP::deg);
  }
  ...
  fDetectorLogic = new G4LogicalVolume(DetectorSolid, fSi, "DetectorLogic");
