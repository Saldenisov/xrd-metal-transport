...
void SAXSEventAction::EndOfEventAction(const G4Event* aEvent)
{
  ...
  if (fSensitiveDetectorHC) {
    ...
    for (size_t i = 0; i < vNumberOfHit; i++) {
      ...
      if (SAXSImageOnly()) {
        if (aHit->GetType() == 0 && aHit->GetTrackID() == 1 && aHit->GetMom().z() > 0.) {
          const double xMm = aHit->GetPos().x() / CLHEP::mm;
          const double yMm = aHit->GetPos().y() / CLHEP::mm;
          analysisManager->FillH2(0, xMm, yMm);
          const double halfWidthMm = 0.0005 * SAXSImagePixels() * SAXSImagePitchUm();
          if (std::abs(xMm) < halfWidthMm && std::abs(yMm) < halfWidthMm) {
            const int channel = fNCi > 0 ? 3 : fNRi > 1 ? 2 : fNRi == 1 ? 1 : 0;
            analysisManager->FillH1(0, channel);
          }
        }
        continue;
      }
